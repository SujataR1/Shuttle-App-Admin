...
const AdminLogin = () => {
  ...
  const getDeviceInfo = () => {
    ...
    if (platform) {
      ...
      else if (platform.includes("iPhone")) platformName = "iOS";
      else if (platform.includes("iPad")) platformName = "iOS";
    ...
    if (userAgent.includes("iPhone")) deviceFamily = "iPhone";
    else if (userAgent.includes("iPad")) deviceFamily = "iPad";
    else if (userAgent.includes("Android")) {
      const androidMatch = userAgent.match(/Android; ([^;]+)/);
      if (androidMatch && androidMatch[1]) {
        deviceFamily = androidMatch[1];
      } else {
        deviceFamily = "Android Device";
      }
    } else if (userAgent.includes("Mac")) deviceFamily = "Mac";
    else if (userAgent.includes("Windows")) deviceFamily = "Windows PC";
    else if (userAgent.includes("Linux")) deviceFamily = "Linux PC";
    
    // Create device name
    let deviceName = `Admin Web - ${browserName} on ${platformName}`;
    
    // Get screen resolution for better device identification
    const screenResolution = `${window.screen.width}x${window.screen.height}`;
    deviceName += ` (${screenResolution})`;
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.log("Device Info being sent:", deviceInfo);

      const res = await axios.post(`${API_BASE}/auth/login`, {
        email: email,
        otp: finalOtp,
        role: "admin",
        device: deviceInfo  // Added device information
      });
